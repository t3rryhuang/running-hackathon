...
func (s *Server) handleCheckinNow(w http.ResponseWriter, r *http.Request, u *User) {
	...
	if err := s.checkinOn(u, channel); err != nil {
		...
		writeJSON(w, http.StatusBadGateway, map[string]any{"ok": false, "error": "Could not reach you just now. Try again in a minute."})
		return
	}
	...
	writeJSON(w, http.StatusOK, map[string]any{"ok": true, "channel": channel, "message": msg})
}
